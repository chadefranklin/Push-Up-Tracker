...
@interface GroupGoalsViewController ()
...
@implementation GroupGoalsViewController
...
- (void)fetchGoals{
    PFRelation *relation = [self.group relationForKey:@"goals"];
...
- (void)didAddGoal:(NSNumber *)pushupAmount withDeadline:(NSDate *)deadline{
    ...
    newGoal = [Goal createGoalForGroup:self.group withPushupTarget:(NSNumber *)pushupAmount withDeadline:deadline withCompletion:^(BOOL succeeded, NSError * _Nullable error) {
        if(succeeded){
